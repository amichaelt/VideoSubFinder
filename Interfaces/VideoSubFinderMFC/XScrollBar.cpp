...
BOOL CXScrollBar::OnSetCursor(CWnd* /*pWnd*/, UINT /*nHitTest*/, UINT /*message*/)
{
	...
	else
	{
		...
		CRect rectLeftArrow(0, 0, m_nBitmapWidth, m_rectClient.Height());
		CRect rectRightArrow(m_rectClient.Width() - m_nBitmapWidth, 0, m_rectClient.Width(), m_rectClient.Height());

		if (rectRightArrow.PtInRect(pt)) ::SetCursor(m_hCursor);
		else if (rectLeftArrow.PtInRect(pt)) ::SetCursor(m_hCursor);
