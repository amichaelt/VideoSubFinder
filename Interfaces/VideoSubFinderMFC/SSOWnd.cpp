...
BEGIN_MESSAGE_MAP(CSSOWnd, CMDIChildWnd)
	ON_NOTIFY(TCN_SELCHANGE, ID_TAB, OnTcnSelchangeTab)
	ON_WM_SIZE()
END_MESSAGE_MAP()

void CSSOWnd::Init(CMainFrame* pParent)
{
	CString strPClass;

	m_pParent = pParent;

	strPClass = AfxRegisterWndClass(CS_VREDRAW | CS_HREDRAW,
...
void CSSOWnd::ResizeControls()
{
	...
	//m_VideoBox.MoveWindow(80, 20, 420, 400);
	//m_VideoBox.ResizeControls();
}
