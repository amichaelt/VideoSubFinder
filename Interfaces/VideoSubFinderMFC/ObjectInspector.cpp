...
void CObjectInspector::CProperty::AddProperty(CProperty *pProperty)
{
	...
	pProperty->m_nIndex     = (int)m_Childs.size();
...
void CObjectInspector::DrawItem(CDC*pDC, CProperty *pProperty, int& nY, int& nCellHeight)
{
	...
	CRect rtPropertyRect, rtValueRect;

	if (pProperty->GetType () != ptGroup)
	{
		rtPropertyRect = CRect(m_rtClientRect.left + 1, nY + 1, m_rtSplitterRect.left - 1, nY + nCellHeight - 1);
		rtValueRect = CRect(m_rtSplitterRect.right + 1, nY + 1, m_rtClientRect.right, nY + nCellHeight - 1);
	}
	else
	{
		rtPropertyRect = CRect(m_rtClientRect.left + 2, nY + 1, m_rtClientRect.right-3, nY + nCellHeight - 1);
		rtValueRect = CRect(m_rtClientRect.right, nY + 1, m_rtClientRect.right, nY + nCellHeight - 1);
	}
	...
	if (pProperty->GetType () == ptGroup)
	{
		...
		if (m_bW2KStyle)
		{
			if (pProperty->m_isColor == true) pDC->FillSolidRect(RectCopyInflate(pProperty->m_rtPropertyRect,0,0,1,1), pProperty->m_clColor);
			else pDC->FillSolidRect(RectCopyInflate(pProperty->m_rtPropertyRect,0,0,1,1), m_clPropertyHPColor);
			pDC->SetTextColor(m_clPropertyHPTColor);		

		}		
		else
			pDC->SetTextColor(m_clPropertyHPTColor);

		pDC->DrawText(pProperty->m_cName, RectCopyDeflate(rtPropertyRect,nBaseIndent + (m_nIndent * 10),0,0,0), SS_CENTER | DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS);
	}
	else
	{
		if (!pProperty->m_bEditing) 
		{
			pDC->FillSolidRect(RectCopyInflate(pProperty->m_rtPropertyRect,0,-1,0,0), m_clPropertyCPColor);
			pDC->FillSolidRect(RectCopyInflate(pProperty->m_rtValueRect,0,-1,0,0), m_clPropertyCVPColor);
		}
...
void CObjectInspector::CalculateScrollRange(CProperty *pProperty)
{
	...
	else
	{
		...
		if (pProperty->m_Childs.size() != 0)
			for (int i = 0; i < (int)pProperty->m_Childs.size(); i++)
					CalculateScrollRange(pProperty->m_Childs[i]);
	}
...
void CObjectInspector::OnPaint() 
{
	...
	memDC.SetBkColor(::GetSysColor (COLOR_BTNFACE));//RGB(127, 255, 0)
...
void CObjectInspector::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	else if (point.y <= m_rtPropertyHeaderRect.bottom)
	{
		/*m_bSorted = true;
...
void CObjectInspector::SetPropertyHPTColor(COLORREF clPropertyHPTColor)
{
	m_clPropertyHPTColor = clPropertyHPTColor;
	Invalidate();
}
void CObjectInspector::SetPropertyHPColor(COLORREF clPropertyHPColor)
{
	m_clPropertyHPColor = clPropertyHPColor;
	Invalidate();
}

void CObjectInspector::SetPropertyCPColor(COLORREF clPropertyCPColor)
{
	m_clPropertyCPColor = clPropertyCPColor;
	Invalidate();
}

void CObjectInspector::SetPropertyCVPColor(COLORREF clPropertyCVPColor)
{
	m_clPropertyCVPColor = clPropertyCVPColor;
	Invalidate();
}
...
CObjectInspector::CProperty *CObjectInspector::GetNextProperty(CProperty *pProperty)
{
	...
	else
	{
		...
		CObjectInspector::CProperty* prop = *pos;

		if (prop->m_Childs.size() > 0)
		{
			return prop->m_Childs[0];
		}
		else 
		{
			if (*pos != m_PropertyList.back())
...
CObjectInspector::CProperty *CObjectInspector::GetPrevProperty(CProperty *pProperty)
{
	...
	else
	{
		...
		if (*pos != m_PropertyList.front())
		{
			...
			if (prop->m_Childs.size() > 0)
			{
				return prop->m_Childs[prop->m_Childs.size()-1];
			}
			else 
				return prop;
		}
...
{
	if (m_pEditingProperty != NULL && pMsg->message == WM_KEYDOWN)
	{
		...
		if (nVirtKey == VK_DOWN)
		{
			...
			while ( (pProperty != NULL) && (pProperty->GetType () == ptGroup) )
			{
				if ((m_nScrollPos + m_nCellHeight) <= m_nScrollRange)
					m_nScrollPos += m_nCellHeight;
				else
				...
				pProperty = GetNextProperty(pProperty);
			}
		...
		if (nVirtKey == VK_UP)
		{
			...
			while ( (pProperty != NULL) && (pProperty->GetType () == ptGroup) )
			{
				...
				pProperty = GetPrevProperty(pProperty);
			}
...
BOOL CObjectInspector::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	...
	if (!m_rtClientRect.PtInRect(point))
	{
		pWnd->SetFocus();
		return true;
	}
...
void CObjectInspector::OnTimer(UINT nIDEvent)
{
	CPoint point;
	CWnd *pWnd;
